...
namespace Rivet {
  ...
  class HiggsDecayParTemplate_fl : public Analysis
  {
  public:
    DEFAULT_RIVET_ANALYSIS_CTOR(HiggsDecayParTemplate_fl);
    ...
    double offshell = 999.0; bool findZ1 = false; bool passZ1 = false; bool makeCuts = true; bool pt_pass=false ; bool max_one; bool max_two;
    ...
    for(int i=0; i<N;i++){
        for(int j=i+1; j<N; j++){
            ...
            for(int k=0; k<N;k++){//test if lepton one has highest pt
                if(i != k && GENlep_id[i] == GENlep_id[k] && GENlep_pt[i] > GENlep_pt[k] ){
                    max_one = true;
                }
            }

           for(int l=0; l<N;l++){//test if lepton two has highest pt
                if(j != l && GENlep_id[j] == GENlep_id[l] && GENlep_pt[j] > GENlep_pt[l] ){
                    max_two = true;
                }
            }
            ...
            }//end of max_one && max_two condidtion, if we made it here then Z1 candidate is made of two highest pt pairs
  ...
  DECLARE_RIVET_PLUGIN(HiggsDecayParTemplate_fl);
}
