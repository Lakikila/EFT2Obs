...
namespace Rivet {
  ...
  class HiggsDecayParTemplate_nc_corect : public Analysis
  {
  public:
    DEFAULT_RIVET_ANALYSIS_CTOR(HiggsDecayParTemplate_nc_corect);

    void init() {

      book(hist_pT_Higgs,   "pT_Higgs",400,0,400);
      book(hist_mz2, "mz2",120, 0, 120);
      book(hist_mz1, "mz1",120, 0, 120);
    }
    ...
    for ( const GenParticle *prtcl : Rivet::HepMCUtils::particles(event.genEvent()) ){
            if(prtcl->status()==1 && ( prtcl->pdg_id()==13 || prtcl->pdg_id()== - 13 || prtcl->pdg_id()==11 || prtcl->pdg_id()== - 11) ){
    ...
    //FourMomentum l3, l4;
    //l3.setEtaPhiMPt(GENlep_eta[L3],GENlep_phi[L3],GENlep_mass[L3],GENlep_pt[L3]);
    //l4.setEtaPhiMPt(GENlep_eta[L4],GENlep_phi[L4],GENlep_mass[L4],GENlep_pt[L4]);
    //FourMomentum ml3l4 = l3+l4;
    //mZ2=ml3l4.mass();
    //mZ2 = Z2.mass();
  ...
  DECLARE_RIVET_PLUGIN(HiggsDecayParTemplate_nc_corect);
}
